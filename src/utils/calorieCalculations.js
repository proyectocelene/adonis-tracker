...
// Categorización biomecánica y metabólica por patrones de movimiento
const MOVEMENT_METRICS = {
  lower_compound: {
    name: 'Compuesto Tren Inferior (Máxima Masa Muscular)',
    met: 7.0,
    displacementMeters: 0.65, // Recorrido medio en sentadilla, prensa, hack
    restSeconds: 120
  },
  upper_compound: {
    name: 'Compuesto Torso (Pectoral, Espalda, Deltoides)',
    met: 5.5,
    displacementMeters: 0.45, // Recorrido medio en press banca, jalón, remo
    restSeconds: 90
  },
  isolation: {
    name: 'Aislamiento Monoarticular (Bíceps, Tríceps, Elevaciones)',
    met: 3.8,
    displacementMeters: 0.35, // Recorrido en poleas, mancuernas, curls
    restSeconds: 75
  }
};
...
export function calculateSetCalories(weightLbs, reps, patternKey, userWeightKg = 78.55) {
  const pattern = MOVEMENT_METRICS[patternKey] || MOVEMENT_METRICS.isolation;
  ...
  // 2. Gasto Metabólico Mecánico:
  // 1 kcal = 4184 Joules. Eficiencia mecánica del músculo humano ~22% (0.22)
  const mechanicalKcal = mechanicalWorkJoules / (4184 * 0.22);

  // 3. Gasto Metabólico Activo durante la ejecución de la serie:
  // Duración estimada de repetición controlada: 3.0 segundos (1.5s concéntrico + 1.5s excéntrico)
  const setDurationMinutes = (numReps * 3.0) / 60;
  // Fórmula estándar MET: Kcal/min = (MET * 3.5 * pesoKg) / 200
  const activeMetabolicKcal = ((pattern.met * 3.5 * userWeightKg) / 200) * setDurationMinutes;

  // 4. Gasto en la Pausa Intra-Serie (Resíntesis de Fosfocreatina ATP-PCr y remoción de lactato):
  // La tasa metabólica de recuperación en descanso es de ~2.0 METs
  const restMinutes = pattern.restSeconds / 60;
  const restRecoveryKcal = ((2.0 * 3.5 * userWeightKg) / 200) * restMinutes;
 ...
 */
export function calculateWorkoutCalories(workoutExercises = {}, userWeightKg = 78.55, definitionsMap = {}, actualDurationMinutes = null) {
  ...
  // Si el usuario descansó más tiempo (espera de máquina, cronómetro propio prolongado):
  const nominalMinutes = Math.max(10, Math.round((completedSetsCount * 2.2) + cardioMinutes));
  let extraRestKcal = 0;
  ...
  if (actualDurationMinutes && actualDurationMinutes > 0) {
    ...
    if (finalDurationMinutes > nominalMinutes) {
      // Minutos de permanencia activa extra en el gimnasio a tasa de bipedestación / recuperación (~1.4 METs)
      const extraMinutes = finalDurationMinutes - nominalMinutes;
      extraRestKcal = ((1.4 * 3.5 * userWeightKg) / 200) * extraMinutes;
  ...
  // En entrenamiento de hipertrofia con sobrecarga progresiva intensa, el EPOC añade entre un 10% y 14%
