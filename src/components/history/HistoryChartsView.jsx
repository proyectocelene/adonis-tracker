...
export default function HistoryChartsView({
  ...
  const totalCaloriesBurned = useMemo(() => {
    return (workoutHistory || []).reduce((acc, ses) => {
      if (ses.isRestDay || ses.isMissedDay) return acc;
      if (ses.calories?.totalKcal) return acc + ses.calories.totalKcal;
      if (ses.exercises) {
        const calc = calculateWorkoutCalories(ses.exercises);
        return acc + calc.totalKcal;
      }
      return acc;
    }, 0);
  }, [workoutHistory]);
  ...
  return (
    <div className="animate-fade">
      {/* KPIs Clínicos con Gasto Calórico Acumulado */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', marginBottom: '18px' }}>
        <div className="card" style={{ padding: '12px 4px', textAlign: 'center', margin: 0 }}>
          ...
          <strong style={{ fontSize: '19px', color: '#0f172a', display: 'block', margin: '2px 0', fontWeight: '800' }}>{totalSessions}</strong>
          <span style={{ fontSize: '10px', color: '#00b464', fontWeight: '800' }}>Archivadas</span>
        </div>
        
        <div className="card" style={{ padding: '12px 4px', textAlign: 'center', margin: 0, borderTop: '4px solid #0066ff' }}>
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: '800', textTransform: 'uppercase' }}>Volumen</span>
          <strong style={{ fontSize: '16px', color: '#0066ff', display: 'block', margin: '2px 0', fontWeight: '800' }}>{totalVolumeLifted.toLocaleString()}</strong>
          <span style={{ fontSize: '10px', color: '#64748b', fontWeight: '700' }}>Lbs-Reps</span>
        </div>

        <div className="card" style={{ padding: '12px 4px', textAlign: 'center', margin: 0, borderTop: '4px solid #ea580c' }}>
          <span style={{ fontSize: '10px', color: '#c2410c', fontWeight: '800', textTransform: 'uppercase' }}>Gasto Kcal</span>
          <strong style={{ fontSize: '16px', color: '#ea580c', display: 'block', margin: '2px 0', fontWeight: '800' }}>{totalCaloriesBurned.toLocaleString()}</strong>
          <span style={{ fontSize: '10px', color: '#9a3412', fontWeight: '700' }}>Kcal Total</span>
        </div>
        
        <div className="card" style={{ padding: '12px 4px', textAlign: 'center', margin: 0 }}>
          <span style={{ fontSize: '10px', color: 'var(--text-muted)', fontWeight: '800', textTransform: 'uppercase' }}>Intensidad</span>
          <strong style={{ fontSize: '19px', color: '#f59e0b', display: 'block', margin: '2px 0', fontWeight: '800' }}>RPE {globalAverageRPE}</strong>
          <span style={{ fontSize: '10px', color: '#64748b', fontWeight: '700' }}>Medio</span>
        </div>
      </div>
