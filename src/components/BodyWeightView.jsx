...
export default function BodyWeightView() {
  ...
  // Estados sincronizados de báscula de bioimpedancia y metas Adonis
  ...
  // Campos opcionales de bioimpedancia para el pesaje
  const [formFatPct, setFormFatPct] = useState('');
  const [formMusclePct, setFormMusclePct] = useState('');
  const [formWaistCm, setFormWaistCm] = useState('');
  const [formShouldersCm, setFormShouldersCm] = useState('');
  ...
  const handleStartEdit = (entry) => {
    ...
    setFormFatPct(entry.bodyFatPct != null ? entry.bodyFatPct.toString() : '');
    setFormMusclePct(entry.skeletalMusclePct != null ? entry.skeletalMusclePct.toString() : '');
    setFormWaistCm(entry.waistCm != null ? entry.waistCm.toString() : '');
    setFormShouldersCm(entry.shouldersCm != null ? entry.shouldersCm.toString() : '');
    setShowAddForm(true);
  };
  ...
  const handleSaveWeight = (e) => {
    ...
    const parsedFat = parseFloat(formFatPct);
    const parsedMuscle = parseFloat(formMusclePct);
    const parsedWaist = parseFloat(formWaistCm);
    const parsedShoulders = parseFloat(formShouldersCm);
    ...
    const entryData = {
      ...
      ...(!isNaN(parsedFat) && parsedFat > 0 ? { bodyFatPct: parsedFat } : {}),
      ...(!isNaN(parsedMuscle) && parsedMuscle > 0 ? { skeletalMusclePct: parsedMuscle } : {}),
      ...(!isNaN(parsedWaist) && parsedWaist > 0 ? { waistCm: parsedWaist } : {}),
      ...(!isNaN(parsedShoulders) && parsedShoulders > 0 ? { shouldersCm: parsedShoulders } : {})
    };
    ...
    // Sincronizar automáticamente en bodyComposition
    const weightInKg = formUnit === 'lbs' ? parseFloat((parsedWeight / 2.20462).toFixed(2)) : parsedWeight;
    setBodyComposition(prev => normalizeBodyComposition({
      ...prev,
      weightKg: weightInKg,
      ...(!isNaN(parsedFat) && parsedFat > 0 ? { bodyFatPct: parsedFat } : {}),
      ...(!isNaN(parsedMuscle) && parsedMuscle > 0 ? { skeletalMusclePct: parsedMuscle } : {}),
  ...
  // Guardar datos desde el modal completo de Báscula Bioimpedancia
  const handleSaveScaleData = (scaleFormData, scaleGoalData) => {
    const normalized = normalizeBodyComposition(scaleFormData);
    setBodyComposition(normalized);
    if (scaleGoalData) setPhysiqueGoal(scaleGoalData);

    // Sincronizar en la bitácora de pesajes para que la gráfica y lista se actualicen de inmediato
    const newWeight = parseFloat(normalized.weightKg);
    if (!isNaN(newWeight) && newWeight > 0) {
      ...
      const scaleEntry = {
        id: Date.now(),
        weight: preferredUnit === 'lbs' ? parseFloat((newWeight * 2.20462).toFixed(1)) : newWeight,
        unit: preferredUnit,
        date: todayDateStr,
        time: nowTimeStr,
        dateString: dateFormatted,
        ...
        moment: 'ayunas',
        comment: 'Sincronizado desde Báscula Bioimpedancia',
        bodyFatPct: normalized.bodyFatPct,
        skeletalMusclePct: normalized.skeletalMusclePct,
        waistCm: normalized.waistCm,
        shouldersCm: normalized.shouldersCm
      };

      setBodyMetrics(prev => {
        const currentList = Array.isArray(prev) ? prev : [];
        const existingIndex = currentList.findIndex(e => e.date === todayDateStr && e.comment?.includes('Báscula'));
        let updated;
        if (existingIndex >= 0) {
          updated = [...currentList];
          updated[existingIndex] = { ...updated[existingIndex], ...scaleEntry, id: updated[existingIndex].id };
        } else {
          updated = [scaleEntry, ...currentList];
        }
        return updated.sort((a, b) => new Date(b.timestamp || b.date).getTime() - new Date(a.timestamp || a.date).getTime());
      });
    }

    modal.showAlert({
      title: "✅ Báscula Sincronizada",
      message: "Todos los datos de bioimpedancia y medidas fueron guardados exitosamente.",
      variant: "success"
    });
  };
  ...
  return (
    <div className="container animate-fade" style={{ paddingBottom: '110px' }}>
      ...
      {/* 3. BÁSCULA INTELIGENTE & COMPOSICIÓN CORPORAL (BIOIMPEDANCIA & PREDICTOR FISIOLÓGICO) */}
      <BodyCompositionCard
      ...
      <WeightEntryModal
        ...
        formFatPct={formFatPct}
        setFormFatPct={setFormFatPct}
        formMusclePct={formMusclePct}
        setFormMusclePct={setFormMusclePct}
        formWaistCm={formWaistCm}
        setFormWaistCm={setFormWaistCm}
        formShouldersCm={formShouldersCm}
        setFormShouldersCm={setFormShouldersCm}
        onOpenScaleModal={() => setShowScaleModal(true)}
      ...
      {/* 7. MODAL DE BÁSCULA INTELIGENTE Y BIOIMPEDANCIA */}
      <BodyCompositionModal
        isOpen={showScaleModal}
        onClose={() => setShowScaleModal(false)}
        initialData={bodyComposition}
        initialGoals={physiqueGoal}
        onSave={handleSaveScaleData}
      />
