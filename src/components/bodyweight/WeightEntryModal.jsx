...
import { Scale, Plus, X, Sparkles, ChevronDown, ChevronUp, Flame, Dumbbell } from 'lucide-react';
...
export default function WeightEntryModal({
  ...
  const [showScaleFields, setShowScaleFields] = React.useState(false);
  if (!showAddForm) {
    return (
      <button
        type="button"
  ...
  return (
    <div className="card animate-fade" style={{ padding: '18px', borderRadius: '22px', background: '#ffffff', border: '2px solid #0066ff', marginBottom: '20px', boxShadow: '0 10px 25px rgba(0, 102, 255, 0.1)' }}>
      ...
      <form onSubmit={handleSaveWeight}>
        ...
        {/* SECCIÓN DESPLEGABLE DE BIOIMPEDANCIA & MEDIDAS (OPCIONAL) */}
        <div style={{ marginBottom: '14px', border: '1.5px solid #e2e8f0', borderRadius: '16px', overflow: 'hidden', background: '#f8fafc' }}>
          <button
            type="button"
            onClick={() => setShowScaleFields(prev => !prev)}
            style={{
              width: '100%',
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span>Datos de Báscula (% Grasa, % Músculo, Medidas)</span>
              <span style={{ fontSize: '10px', color: '#64748b', fontWeight: 'normal' }}>[Opcional]</span>
            </span>
            {showScaleFields || formFatPct || formMusclePct ? (
          ...
          {(showScaleFields || formFatPct || formMusclePct || formWaistCm || formShouldersCm) && (
            <div style={{ padding: '12px 14px', borderTop: '1px solid #e2e8f0', background: '#ffffff' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                <div>
                  <label style={{ fontSize: '10.5px', fontWeight: '800', color: '#475569', display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                    <Flame size={12} color="#f97316" /> % Grasa Corporal
                  </label>
                  <input
                    type="number"
                    step="0.1"
                    placeholder="Ej. 18.2"
                    value={formFatPct}
                    onChange={(e) => setFormFatPct && setFormFatPct(e.target.value)}
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: '10px',
                ...
                <div>
                  <label style={{ fontSize: '10.5px', fontWeight: '800', color: '#475569', display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                    <Dumbbell size={12} color="#0066ff" /> % Músculo Esquelético
                  </label>
                  <input
                    type="number"
                    step="0.1"
                    placeholder="Ej. 42.5"
                    value={formMusclePct}
                    onChange={(e) => setFormMusclePct && setFormMusclePct(e.target.value)}
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: '10px',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px' }}>
                <div>
                  <label style={{ fontSize: '10.5px', fontWeight: '800', color: '#475569', display: 'block', marginBottom: '4px' }}>
                    📏 Cintura (cm)
                  </label>
                  <input
                    type="number"
                    ...
                    placeholder="Ej. 82"
                    value={formWaistCm}
                    onChange={(e) => setFormWaistCm && setFormWaistCm(e.target.value)}
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: '10px',
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.5"
                    placeholder="Ej. 122"
                    value={formShouldersCm}
                    onChange={(e) => setFormShouldersCm && setFormShouldersCm(e.target.value)}
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: '10px',
              ...
              {onOpenScaleModal && (
                ...
                >
                  <Scale size={13} /> Abrir Báscula Completa (18 Parámetros)
                </button>
              )}
            </div>
          )}
