            ...
            return (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {/* CHIPS DE BÁSCULA INTELIGENTE / BIOIMPEDANCIA */}
                  {(entry.bodyFatPct || entry.skeletalMusclePct || entry.waistCm || entry.shouldersCm) && (
                    <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginTop: '4px' }}>
                      {entry.bodyFatPct && (
                        <span style={{ fontSize: '10px', background: '#fff7ed', color: '#c2410c', border: '1px solid #ffedd5', padding: '1px 6px', borderRadius: '6px', fontWeight: '800' }}>
                          🔥 {entry.bodyFatPct}% Grasa
                        </span>
                      )}
                      {entry.skeletalMusclePct && (
                        <span style={{ fontSize: '10px', background: '#eff6ff', color: '#1d4ed8', border: '1px solid #dbeafe', padding: '1px 6px', borderRadius: '6px', fontWeight: '800' }}>
                          💪 {entry.skeletalMusclePct}% Músculo
                        </span>
                      )}
                      {entry.waistCm && (
                        <span style={{ fontSize: '10px', background: '#f8fafc', color: '#475569', border: '1px solid #e2e8f0', padding: '1px 6px', borderRadius: '6px', fontWeight: '800' }}>
                          📏 Cintura: {entry.waistCm}cm
                        </span>
                      )}
                      {entry.shouldersCm && (
                        <span style={{ fontSize: '10px', background: '#f8fafc', color: '#475569', border: '1px solid #e2e8f0', padding: '1px 6px', borderRadius: '6px', fontWeight: '800' }}>
                          📐 Hombros: {entry.shouldersCm}cm
                        </span>
                      )}
                    </div>
                  )}
