...
export default function HistoryView() {
  ...
  const latestWeightObj = Array.isArray(bodyMetrics) && bodyMetrics.length > 0 
    ? [...bodyMetrics].reverse().find(m => m && (m.weight || m.peso))
    : null;
  const userBodyWeight = latestWeightObj ? parseFloat(latestWeightObj.weight || latestWeightObj.peso) : 180;
  ...
  // SANITIZACIÓN CLÍNICA DE HISTORIAL EN LA NUBE
  const handleSanitizeHistory = () => {
    modal.showConfirm({
      title: "🩺 ¿Sanitizar Inconsistencias en Firebase?",
      message: "Esta acción corregirá permanentemente en la nube y en la memoria local:\n\n1. Eliminación de las 12 sesiones de prueba de julio de 1,000 lbs.\n2. Corrección de la inversión de peso/reps en Leg Curl del 7 de agosto (pasará a 90 lbs x 10 reps).\n3. Corrección del error tipográfico de 115 reps en Press Militar del 31 de agosto (pasará a 15 reps).\n\nTus marcas reales y datos fidedignos no se verán afectados.",
      confirmText: "🩺 Sí, Sanitizar Ahora",
      cancelText: "Cancelar",
      variant: "warning",
      onConfirm: async () => {
        try {
          setIsRestoring(true);
          const res = await sanitizeCloudHistory(currentUser, (msg) => setRestoreStatus(msg));
          setIsRestoring(false);
          modal.showAlert({
            title: "🎉 Historial Sanitizado con Éxito",
            message: `Operación completada:\n• Sesiones de prueba eliminadas: ${res.deletedSyntheticCount}\n• Sesiones corregidas con precisión: ${res.fixedSessionsCount}\n\nTus analíticas de 1RM y volumen ahora reflejan datos 100% limpios.`,
            variant: "success"
          });
        } catch (err) {
          setIsRestoring(false);
          modal.showAlert({ title: "Error al Sanitizar", message: err.message, variant: "danger" });
        }
      }
    });
  };
  ...
  return (
    <div className="container" style={{ paddingBottom: '70px' }}>
      ...
      <DataBackupModal
        ...
        handleSanitizeHistory={handleSanitizeHistory}
        onOpenRoutineModal={() => {
          setShowConfigModal(false);
